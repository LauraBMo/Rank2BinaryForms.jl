...
@testset "Rank2BinaryForms.jl" begin
    ...
    @testset "Building polynomials" begin
        ...
        @testset "Nemo fmpz" begin
            ...
            @test first(r1) == 1
            @test buildpolynomial(vars)(r1) == vars[1]^3 * vars[2] - vars[1] * vars[2]^3

            r2 = introots.(affineGamma(4)([1,2,4]))
            @test first(r2) == -2
            @test buildpolynomial(vars)(r2) == vars[1]^3 * vars[2] + 3 * vars[1]^2 * vars[2]^2 + 2 * vars[1] * vars[2]^3
